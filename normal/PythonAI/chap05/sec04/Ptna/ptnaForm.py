...
def chagImg(img):
    ...
    canvas.itemconfig(
    on_canvas,
    image = my_images[img],   # イメージオブジェクトを指定
    )
    canvas.update()
...
def run():
    # グローバル変数を使用するための記述
    global entry, response_area, lb, action, canvas, on_canvas, my_images
    ...
    # イメージを用意
    my_images.append(tk.PhotoImage(file = "talk.gif")) 
    my_images.append(tk.PhotoImage(file = "empty.gif"))
    my_images.append(tk.PhotoImage(file = "angry.gif"))
    my_images.append(tk.PhotoImage(file = "happy.gif"))
    ...
    on_canvas = canvas.create_image(
        0,                                  # x座標
        0,                                  # y座標
        image = my_images[0],               # 配置するイメージオブジェクトを指定
