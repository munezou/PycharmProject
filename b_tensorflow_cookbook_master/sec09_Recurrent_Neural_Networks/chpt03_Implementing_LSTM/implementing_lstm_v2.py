...
class LSTM_Model():
	def __init__(
			self, embedding_size, rnn_size, batch_size, learning_rate, training_seq_len, vocab_size, infer_sample=False
	):
		...
		with tf.compat.v1.variable_scope('lstm_vars'):
			...
			W = tf.compat.v1.get_variable(
				'W', [self.rnn_size, self.vocab_size], tf.float32, tf.compat.v1.random_normal_initializer()
			)
		...
		outputs, last_state = decoder(
			rnn_inputs_trimmed, self.initial_state, self.lstm_cell,
		...
		loss = loss_fun(
			[self.logit_output], [tf.reshape(self.y_output, [-1])], [tf.ones([self.batch_size * self.training_seq_len])]
		)
...
with tf.compat.v1.variable_scope(tf.compat.v1.get_variable_scope(), reuse=True):
	test_lstm_model = LSTM_Model(
		embedding_size, rnn_size, batch_size, learning_rate, training_seq_len, vocab_size, infer_sample=True
	)
...
for epoch in range(epochs):
	...
	for ix, batch in enumerate(batches):
		...
		temp_loss, state, _ = sess.run(
			[lstm_model.cost, lstm_model.final_state, lstm_model.train_op], feed_dict=training_dict
		)
