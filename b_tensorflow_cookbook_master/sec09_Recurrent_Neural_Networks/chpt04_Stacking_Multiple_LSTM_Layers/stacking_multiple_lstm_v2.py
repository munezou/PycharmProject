...
class LSTM_Model():
    def __init__(self, rnn_size, num_layers, batch_size, learning_rate,
        ...
        self.lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(rnn_size)
        self.lstm_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([self.lstm_cell for _ in range(self.num_layers)])
        ...
        with tf.compat.v1.variable_scope('lstm_vars'):
            ...
            # Define Embedding
            embedding_mat = tf.compat.v1.get_variable('embedding_mat', [self.vocab_size, self.rnn_size],
                                            tf.float32, tf.compat.v1.random_normal_initializer())
                                            
            embedding_output = tf.compat.v1.nn.embedding_lookup(params=embedding_mat, ids=self.x_data)
