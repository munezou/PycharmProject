...
def read_cifar_files(filename_queue, distort_images=True):
	...
	image_extracted = tf.reshape(
		tf.slice(record_bytes, [1], [image_vec_length]), [num_channels, image_height, image_width]
	)
...
def cifar_cnn_model(input_images, batch_size, train_logical=True):
	def truncated_normal_var(name, shape, dtype):
		return (
			tf.compat.v1.get_variable(
				name=name, shape=shape, dtype=dtype, initializer=tf.compat.v1.truncated_normal_initializer(stddev=0.05)
			)
		)
	...
	def zero_var(name, shape, dtype):
		return (
			tf.compat.v1.get_variable(
				name=name, shape=shape, dtype=dtype, initializer=tf.compat.v1.constant_initializer(0.0)
			)
		)
	...
	pool1 = tf.nn.max_pool2d(
		input=relu_conv1, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool_layer1'
	)
	...
	pool2 = tf.nn.max_pool2d(
		input=relu_conv2, ksize=[1, 3, 3, 1], strides=[1, 2, 2, 1], padding='SAME', name='pool_layer2'
	)
...
def train_step(loss_value, generation_num):
	...
	model_learning_rate = tf.compat.v1.train.exponential_decay(
		learning_rate, generation_num, num_gens_to_wait, lr_decay, staircase=True
	)
	# Create optimizer
