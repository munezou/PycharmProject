...
test_xdata = np.array([np.reshape(x, (28, 28)) for x in x_test])
...
full1_weight = tf.Variable(tf.random.truncated_normal(
...
full2_weight = tf.Variable(tf.random.truncated_normal(
...
def my_conv_net(conv_input_data):
	...
	conv1 = tf.nn.conv2d(input=conv_input_data, filters=conv1_weight, strides=[1, 1, 1, 1], padding='SAME')
	relu1 = tf.nn.relu(tf.nn.bias_add(conv1, conv1_bias))
	max_pool1 = tf.nn.max_pool2d(
		input=relu1, ksize=[1, max_pool_size1, max_pool_size1, 1],
	...
	max_pool2 = tf.nn.max_pool2d(
		input=relu2, ksize=[1, max_pool_size2, max_pool_size2, 1],
