...
class OutputGenerator:
    ...
    def generate_filtered_edf(self) -> None:
        ...
        for i in range(int(self.__duration)):
            edf_w.writeSamples(
                [
                    self.__eeg["fp1_ma"][i * sampling_freq : (i + 1) * sampling_freq],
                    self.__eeg["fp2_ma"][i * sampling_freq : (i + 1) * sampling_freq],
                    self.__eeg["fp1_fp2"][i * sampling_freq : (i + 1) * sampling_freq],
                    self.__eeg["m1_m2"][i * sampling_freq : (i + 1) * sampling_freq],
                    self.__eeg["fp1"][i * sampling_freq : (i + 1) * sampling_freq],
                    self.__eeg["fp2"][i * sampling_freq : (i + 1) * sampling_freq],
                    self.__eeg["a1"][i * sampling_freq : (i + 1) * sampling_freq],
                    self.__eeg["a2"][i * sampling_freq : (i + 1) * sampling_freq],
